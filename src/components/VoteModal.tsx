...
const VoteModal: React.FC<VoteModalProps> = ({ isSpecialCategory = false }) => {
  ...
  }, [isSpecialCategory, setSpecialVote, setOfferSecondVote]);
  ...
  const handleDeclineSpecialVote = () => {
    console.log("Declining special vote - Resetting state");
    setSpecialVote(false);
    setOfferSecondVote(false);
    resetSelection(); // Réinitialise également selectedInfluenceur et phoneNumber
  };
  ...
  return (
    <>
      {offerSecondVote && selectedInfluenceur ? (
        <SecondVoteOffer
      ...
      {!offerSecondVote && selectedInfluenceur && !isSpecialCategory ? (
        <div
